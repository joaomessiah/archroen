# Roman-villa set: frozen reports & outputs

The **real-world application** corpus: 30 South Limburg Roman-villa reports the workflow was run on.
This folder keeps a **frozen copy of the source report PDFs and the pipeline outputs** so the thesis
results are preserved even after the live files are regenerated. **27 of the 30 report PDFs are
included** (3 are link-only for copyright, see below); all **30 output CSVs** are kept.

| Folder | What's inside |
|---|---|
| [input_reports/](input_reports/) | The source report PDFs, 27 of 30 (3 are link-only for copyright). |
| [outputs/](outputs/) | One `<report_id>.csv` pottery summary per report (all 30). |

This set has **no gold standards** (it is the application, not the accuracy measurement). A live copy
of the PDFs is under
[input_files/reports/south_limburg_villas/](../../../../input_files/reports/south_limburg_villas/). For the full picture, see the
...
> The extracted-data CSVs for 12726, 12727, 12732 remain under [outputs/](outputs/).
