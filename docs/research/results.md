...
- **Site resolution is where Rules-only collapses** (3.3%). Without an AI step the site name is wrong on
  almost every find — the deterministic site normalization alone can't pick the right place from messy,
  multi-site report text. Both AI modes recover this (Claude 95.9%).
