...
5. [getting_started/how_to_run.md](getting_started/how_to_run.md) — the full guide: batches, modes, and evaluation

## A note on the AI models

The workflow can run in different **modes**. **Rules-only mode** uses no AI at all and is completely
free — a good way to try it first. The other modes (**Claude mode**, **Llama mode**) call a cloud AI
model to improve the results and need an API key. See
[getting_started/api_keys.md](getting_started/api_keys.md) and
