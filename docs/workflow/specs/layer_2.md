...
- **Strips repeating headers/footers** and page furniture. These are detected by finding lines that
  recur across pages. A leading page number is normalized first, so `42 / Foo` and `33 / Foo` collapse
  together.
- **Normalizes archaic spellings** so older orthography matches modern forms.
...
1. **Sectioning.** Splits the cleaned text into logical sections by detecting headings. The detector
   ...
   *pottery / aardewerk*, *dating / datering*, *finds / vondsten*). It also recognizes **numbered
   headings** (`1.`, `2.1.`, `1.2.3.`), with guards so decimals like `2.0` aren't mistaken for headings.
2. **Chunking.** Breaks each section into overlapping character chunks of `CHUNK_SIZE` with
   `CHUNK_OVERLAP`. The overlap means a term near a chunk edge still has context on both sides. Chunks
   bound the context window used by detection and the AI layers.
...
| Setting | Role |
|---|---|
| `CHUNK_SIZE` | Chunk length in characters (default `1200`) |
