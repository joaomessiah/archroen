...
Find candidate mentions of **pottery, periods, and centuries** in the chunked text — using
vocabulary-derived regular expressions, plus trigger words to catch pottery types that aren't in the
pattern list.

## Regex detection (`detection.py`)

- Scans each chunk against the **generated pattern files** in `data/patterns/`
  (`pottery_patterns.json`, `chronology_patterns.json`, `century_patterns.json`) and emits a
  **candidate** per match, carrying its `CONTEXT_WINDOW_CHARS`-wide surrounding context (snapped to
  sentence boundaries — with care not to break on initials like `J. Smith`, numbered list items, or
  `n.Chr.`/`v.Chr.`).
...
1. **Trigger words.** A list in English, Dutch, and Latin (`data/patterns/pottery_triggers.json`) flags
   candidate sentences. Each trigger has a *strength*: `strong` triggers fire on their own; `weak` ones
   need supporting cues nearby.
...
|---|---|---|
| `CONTEXT_WINDOW_CHARS` | — | Width of the context window stored per candidate |
| `POTTERY_EXTRACT_LLM_USE` | `False` | Enable the AI fallback for ambiguous trigger sentences |
| `*_PATTERNS_PATH` | — | The generated pattern files (see [../../reference/data_files.md](../../reference/data_files.md)) |
