...
**the pottery summary**. This is one row per distinct pottery find the report mentions, with the find's
name, typology, quantity, find site, and the **date range** it belongs to. The focus is the **Roman
period**.
...
site resolution is purely string-based. Every model-produced find must also carry a **verbatim quote**
from the report (anti-hallucination). And when run with no model at all (**Rules-only mode**), the same
rule pipeline becomes a fully deterministic, free baseline.

How much the model is used is set by one master switch, `WORKFLOW_MODE`, with four settings:
**Claude mode** (`claude`) and **Llama mode** (`cloud-llama`), the cloud hybrids; a local-Llama option
(`local-llama`, which runs on the local Ollama model `LLM_MODEL`, default `llama3.2:1b`); and
**Rules-only mode** (`rules-only`, no AI). The trade-offs and the measured comparison live in
